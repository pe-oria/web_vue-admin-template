...
<template>
  <div>
    ...
    <el-button type="primary" @click="addVisible = true">新增安排</el-button>
    <el-dialog title="新增安排" :visible.sync="addVisible">
      <el-form :model="form">
    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column prop="courseid" label="课程名称" width="180">
      </el-table-column>
      ...
      <el-table-column label="操作" width="500">
        <template slot-scope="scope">
          <el-button
            ...
            >编辑</el-button
          >
          <el-dialog title="修改课程" :visible.sync="changeVisible" width="30%">
            <el-form
              :model="change_form"
  ...
  methods: {
    ...
    new() {
      ...
      const params = {
        ...
        courseid: this.formInline.classes,
      };
      this.axios
        .get("http://localhost:8080/cm", { params })
    ...
    add() {
      const addData = {
        courseid: this.add_form.classname,
        teacher: this.add_form.teacher,
      };
      this.axios
        .post("http://localhost:8080/cm", addData)
        .then((response) => {
    ...
    get_change(id) {
      const url = `http://localhost:8080/cm/${id}`;
      this.cid = id;
      this.axios
        .get(url)
        .then((response) => {
          ...
          this.change_form.classname = list.courseid;
    ...
    change() {
      const changeData = {
        id: this.cid,
        courseid: this.change_form.classname,
        teacher: this.change_form.teacher,
      };
      this.axios
        .put("http://localhost:8080/cm", changeData)
        .then((response) => {
